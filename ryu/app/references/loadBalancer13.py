...
class loadBalancer13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        self.logger.info("\n Packet-In - DPID: %s SMAC: %s DMAC: %s InPort: %s", dpid, src, dst,
                         in_port)
        ...
        if (eth.ethertype == 0x0806):
            ...
            if ((arpContents.dst_ip == "10.0.0.100") and (arpContents.opcode == 1)):
                ...
                arpServer = parser.OFPPacketOut(datapath=datapath, in_port=ofproto.OFPP_ANY,
                                                data=packetReply.data, actions=actionsServer,
                                                buffer_id=0xffffffff)

                self.logger.info(
                    "\n Packet-Out - DPID: %s SMAC: 11:22:33:ab:cd:ef DMAC: %s OutPort: %s", dpid,
                    src, in_port)
        ...
        if (eth.ethertype == 0x0800):
            ...
            if ((ipContents.dst == "10.0.0.100") and (ipContents.proto == 0x06)):
                ...
                match1 = parser.OFPMatch(in_port=in_port, eth_type=eth.ethertype, eth_src=eth.src,
                                         eth_dst=eth.dst, ip_proto=ipContents.proto,
                ...
                actions1 = [parser.OFPActionSetField(ipv4_src="10.0.0.100"),
                            parser.OFPActionSetField(eth_dst=serverMac),
                            parser.OFPActionSetField(ipv4_dst=serverIP),
                            parser.OFPActionOutput(self.serverCount)]
                ...
                flowMod1 = parser.OFPFlowMod(datapath=datapath, match=match1, idle_timeout=7,
                                             instructions=ipInst1, buffer_id=msg.buffer_id,
                                             cookie=cookie1)

                self.logger.info("\n Added flow for Host to Server condition------->")
                self.logger.info(
                    "\n Application on server" + str(self.serverCount) + " - IP: " + str(
                        serverIP) + " Mac: " + str(serverMac))
                ...
                # Perform TCP action only if matching TCP properties
                match2 = parser.OFPMatch(self.serverCount, eth_type=eth.ethertype,
                                         eth_src=serverMac, eth_dst="11:22:33:ab:cd:ef",
                                         ip_proto=ipContents.proto, ipv4_src=serverIP,
                                         ipv4_dst="10.0.0.100", tcp_src=tcpContents.dst_port,
                                         tcp_dst=tcpContents.src_port)
                ...
                actions2 = [parser.OFPActionSetField(eth_src="11:22:33:ab:cd:ef"),
                            ...
                            parser.OFPActionSetField(ipv4_dst=ipContents.src),
        ...
        flowMod2 = parser.OFPFlowMod(datapath=datapath, match=match2, idle_timeout=7,
                                     instructions=ipInst2, cookie=cookie2)
