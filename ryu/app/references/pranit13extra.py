# https://github.com/pranityadav7/Python-Load-Balancer-Application
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.serverlist.append({'ip': "10.0.0.1", 'mac': "00:00:00:00:00:01",
    ...
    def function_for_arp_reply(self, dst_ip,
                               dst_mac):  # Function placed here, source MAC and IP passed from below now become the destination for the reply ppacket
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether.ETH_TYPE_ARP:  # If the ethernet frame has eth type as 2054 indicating as ARP packet..
            ...
            if arp_header.dst_ip == self.virtual_lb_ip and arp_header.opcode == arp.ARP_REQUEST:  # ..and if the destination is the virtual IP of the load balancer and Opcode = 1 indicating ARP Request
                ...
                reply_packet = self.function_for_arp_reply(arp_header.src_ip,
                                                           arp_header.src_mac)  # Call the function that would build a packet for ARP reply passing source MAC and source IP
        ...
        match = parser.OFPMatch(in_port=in_port, eth_type=eth.ethertype, eth_src=eth.src,
                                eth_dst=eth.dst, ip_proto=ip_header.proto, ipv4_src=ip_header.src,
        ...
        match = parser.OFPMatch(in_port=server_outport_selected, eth_type=eth.ethertype,
                                eth_src=server_mac_selected, eth_dst=self.virtual_lb_mac,
                                ip_proto=ip_header.proto, ipv4_src=server_ip_selected,
                                ipv4_dst=self.virtual_lb_ip, tcp_src=tcp_header.dst_port,
        ...
        print("<++++++++Reply sent from server having IP: " + str(
            ...
            ip_header.src) + " via load balancer :" + str(self.virtual_lb_ip) + "++++++++>")
